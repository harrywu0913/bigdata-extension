...
class BucketWriter {
    ...
    private void open() throws IOException, InterruptedException {
        ...
        synchronized (staticLock) {
            ...
            try {
                ...
                LOG.info("#####BucketWriter.open bucketPath:{},targetPath:{}", bucketPath, targetPath);
        ...
        if (rollInterval > 0) {
            Callable<Void> action = new Callable<Void>() {
                public Void call() throws Exception {
                    LOG.info("#####Rolling file ({}): Roll scheduled after {} sec elapsed.",
        ...
        if (!isBatchComplete()) {
            ...
            if (idleTimeout > 0) {
                ...
                if (idleFuture == null || idleFuture.cancel(false)) {
                    LOG.error("#####BucketWriter:flush, idleTimeout:{}", idleTimeout);
    ...
    public synchronized void append(final Event event)
        ...
        LOG.info("#####BucketWriter.append idleFuture:{}.", idleFuture);
        if (idleFuture != null) {
        ...
        // just reuse this one
        LOG.info("#####BucketWriter.append isOpen:{}", isOpen);
        if (!isOpen) {
            if (closed) {
        ...
        if (shouldRotate()) {
            ...
            LOG.info("#####BucketWriter.append isUnderReplicated:{}.", isUnderReplicated);
            if (isUnderReplicated) {
                LOG.info("#####BucketWriter.append maxConsecUnderReplRotations:{}.consecutiveUnderReplRotateCount:{}", maxConsecUnderReplRotations, consecutiveUnderReplRotateCount);
        ...
        LOG.info("#####BucketWriter.append eventCounter:{} batchCounter:{} batchSize:{}.", eventCounter, batchCounter, batchSize);
    ...
    private boolean shouldRotate() {
        ...
        if (writer.isUnderReplicated()) {
            LOG.info("######BucketWriter: shouldRotate: {}", isUnderReplicated);
        ...
        if ((rollCount > 0) && (rollCount <= eventCounter)) {
            ...
            LOG.info("######BucketWriter: shouldRotate: rolling: rollCount: {}, events: {}", rollCount, eventCounter);
            doRotate = true;
        }

        if ((rollSize > 0) && (rollSize <= processSize)) {
            LOG.debug("rolling: rollSize: {}, bytes: {}", rollSize, processSize);
            LOG.info("######BucketWriter: shouldRotate: rolling: rollSize: {}, bytes: {}", rollSize, processSize);
            doRotate = true;
        }
