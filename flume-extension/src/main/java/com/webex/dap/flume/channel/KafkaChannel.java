...
import static com.webex.dap.flume.channel.KafkaChannelConfiguration.*;
...
public class KafkaChannel extends BasicChannelSemantics {
  ...
  */
    ...
    private void configureInterceptors(Context ctx) {
        String interceptorListStr = ctx.getString("interceptors", "");
        ...
        Context interceptorContexts =
                new Context(ctx.getSubProperties("interceptors."));
            ...
            if (!consumerAndRecords.get().failedEvents.isEmpty()) {
                ...
                try {
                    ...
                    if (consumerAndRecords.get().recordIterator.hasNext()) {
                        ...
                        //
                        e = interceptorChain.intercept(e);
