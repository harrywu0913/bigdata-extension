...
public class DapCommandWithDestination extends FsCommand {
    ...
    private EnumSet<org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute> preserveStatus =
            EnumSet.noneOf(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute.class);
     ...
     */
    private boolean shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute attribute) {
    ...
    protected void preserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute fileAttribute) {
        for (org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute attribute : preserveStatus) {
    ...
    protected void copyStreamToTarget(InputStream in, PathData target)
        ...
        try {
            ...
            String extension = targetFs.writeStreamToFile(in, tempTarget, lazyPersist);
//                if (extension != null && extension.trim().length() > 0 && !target.path.getName().endsWith(extension)){
//                    targetFs.rename(tempTarget, target.suffix(extension));
//                }else{
...
//                }
     ...
     * The method calls {@link #shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute)} to check what
    ...
    protected void preserveAttributes(PathData src, PathData target,
            ...
            throws IOException {
        if (shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute.TIMESTAMPS)) {
            target.fs.setTimes(
                    target.path,
                    src.stat.getModificationTime(),
                    src.stat.getAccessTime());
        }
        if (shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute.OWNERSHIP)) {
            target.fs.setOwner(
                    target.path,
                    src.stat.getOwner(),
                    src.stat.getGroup());
        }
        if (shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute.PERMISSION) ||
                shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute.ACL)) {
            target.fs.setPermission(
                    target.path,
                    src.stat.getPermission());
        }
        if (shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute.ACL)) {
        ...
        final boolean preserveXAttrs = shouldPreserve(org.apache.hadoop.fs.shell.CommandWithDestination.FileAttribute.XATTR);
        if (preserveXAttrs || preserveRawXAttrs) {
            Map<String, byte[]> srcXAttrs = src.fs.getXAttrs(src.path);
            if (srcXAttrs != null) {
                Iterator<Map.Entry<String, byte[]>> iter = srcXAttrs.entrySet().iterator();
                while (iter.hasNext()) {
    ...
    private static class TargetFileSystem extends FilterFileSystem {
        ...
        String writeStreamToFile(InputStream in, PathData target,
                                 boolean lazyPersist) throws IOException {
            String extension = "";
            OutputStream out = null;
            try {
                boolean isCompressed = getConf().getBoolean(COMPRESS, false);
                if (isCompressed) {
                    Class<? extends CompressionCodec> codecClass = getOutputCompressorClass(getConf(), GzipCodec.class);
                    CompressionCodec codec = (CompressionCodec) ReflectionUtils.newInstance(codecClass, getConf());
                    extension = codec.getDefaultExtension();

                    out = codec.createOutputStream(create(target, lazyPersist));
                } else {
    ...
    public static final String COMPRESS = "output.compress";
    public static final String COMPRESS_CODEC = "output.compress.codec";

    public static Class<? extends CompressionCodec> getOutputCompressorClass(Configuration conf, Class<? extends CompressionCodec> defaultValue) {
